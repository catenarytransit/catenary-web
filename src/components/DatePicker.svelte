<script lang="ts">
	...
	function advanceTime(minutesToChange: number) {
		...
		let [y, m, d] = dateString ? dateString.split('-').map(Number) : [new Date().getFullYear(), new Date().getMonth() + 1, new Date().getDate()];
	...
	$: displayDate = (() => {
		...
		const canadianZones = [
			...
			'America/Blanc-Sablon', 'America/Creston', 'America/Dawson', 'America/Dawson_Creek', 
			'America/Fort_Nelson', 'America/Glace_Bay', 'America/Goose_Bay', 'America/Inuvik', 
			'America/Iqaluit', 'America/Nipigon', 'America/Pangnirtung', 'America/Rainy_River', 
			'America/Rankin_Inlet', 'America/Resolute', 'America/Swift_Current', 'America/Thunder_Bay', 
...
<div class="flex flex-col mb-2">
	<div class="flex flex-row gap-2 items-center">
		<!-- Date Box -->
		<div class="relative flex items-center border border-gray-400 dark:border-gray-500 bg-white dark:bg-gray-800 text-black dark:text-white px-2 py-1 rounded-[1px] focus-within:border-blue-500 focus-within:ring-1 focus-within:ring-blue-500">
			<!-- Text background changes to blue if it is selected, let's mimic selection if !isNow? Actually, let's just show text -->
			<span class={`font-medium text-sm min-w-[5rem] text-center px-1 ${!isNow ? 'bg-blue-600 text-white dark:bg-blue-500' : ''}`}>{displayDate}</span>
			<span class="material-symbols-outlined ml-2 text-gray-500 text-base pointer-events-none">calendar_today</span>
			...
			{#if isCalendarOpen}
				...
				<div class="absolute top-[110%] left-0 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 shadow-xl rounded z-50 p-4 hidden md:block min-w-[280px]">
					<!-- Header -->
					<div class="flex justify-between items-center mb-4">
						<button class="w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-300 transition-colors" on:click|stopPropagation={() => changeMonth(-1)}>
						...
						<thead>
							<tr>
								{#each weekDays as dayName}
									<th class="text-xs font-semibold text-gray-500 dark:text-gray-400 p-1 w-8">{dayName}</th>
								{/each}
							</tr>
						</thead>
						<tbody>
							{#each calendarGrid as week}
								<tr>
									{#each week as day}
										<td class="p-1">
											{#if day}
												{@const isSelected = dateString && parseInt(dateString.split('-')[0]) === calendarYear && parseInt(dateString.split('-')[1]) === calendarMonth + 1 && parseInt(dateString.split('-')[2]) === day}
												{@const isTodayInTz = currentTzDate.year === calendarYear && currentTzDate.month === calendarMonth && currentTzDate.day === day}
												<button 
													class="w-8 h-8 flex items-center justify-center rounded-full transition-colors mx-auto text-sm outline-none
													{isSelected ? 'bg-blue-600 text-white dark:bg-blue-600 hover:bg-blue-700 dark:hover:bg-blue-500' : 'bg-transparent hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-800 dark:text-gray-200'}
													{isTodayInTz ? 'font-bold underline' : ''}"
													on:click|stopPropagation={() => selectDate(calendarYear, calendarMonth, day)}
												>
													{day}
												</button>
											{/if}
		...
		<!-- Time Box -->
		<div class="flex items-center border border-gray-400 dark:border-gray-500 bg-white dark:bg-gray-800 rounded-[1px] focus-within:border-blue-500 focus-within:ring-1 focus-within:ring-blue-500">
			<button class="px-1 py-1 flex items-center justify-center text-gray-500 hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors dark:text-gray-300" on:click={() => advanceTime(-15)}>
