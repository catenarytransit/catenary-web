<script lang="ts">
	...
	if (urlParams.get('style') == 'midnight') {
		...
		if (urlParams.get('style') == 'nothingdark') {
			style = '/nothing-dark.json';
		}
	...
	<div class="w-full">
		...
		{#if !$isLoading}
			<div
				...
				class="z-20 rounded-t-2xl md:rounded-none fixed bottom-0 w-full sm:w-2/5 md:h-full md:w-[380px] xl:w-[480px] bg-white dark:bg-gray-900 bg-opacity-70 dark:bg-opacity-70 md:bg-opacity-80 md:dark:bg-opacity-70 backdrop-blur-xs md:backdrop-blur-sm md:fixed md:left-0 md:top-0 md:bottom-0 text-black dark:text-white flex flex-col select-text"
			>
				<div
						...
						{#if typeof geolocation.coords == 'object'}
							{#if typeof geolocation.coords.speed == 'number'}
								<div
									...
									class="leading-tight md:text-base rounded-lg text-black bg-white dark:text-white border border-gray-500 dark:bg-gray-800 shadow-sm shadow-gray-400 dark:shadow-gray-700 px-1 py-0.5"
